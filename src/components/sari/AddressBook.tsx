...
import { ArrowLeft, Plus, Home, Briefcase, MapPin, Trash2, X, Check } from 'lucide-react';
...
const COLOR_MAP = { home: 'bg-[#EFF6FF] text-[#2563EB]', work: 'bg-[#F0FDF4] text-[#16A34A]', other: 'bg-[#FDF6EC] text-[#C94A2A]' };
    ...
    <div className="min-h-screen bg-neutral-50 pb-24">
      ...
      <div className="bg-white sticky top-0 z-10 shadow-sm">
        <div className="px-5 py-4 flex items-center gap-3">
          ...
          <button
            ...
            className="w-10 h-10 rounded-full bg-[#C94A2A] flex items-center justify-center shadow-lg active:scale-95"
          >
      ...
      <div className="px-5 pt-5 space-y-3">
        ...
        {addresses.map(addr => {
          ...
          const colorClass = COLOR_MAP[addr.icon];
          return (
        ...
        <button
          onClick={() => setShowModal(true)}
          className="w-full bg-white rounded-2xl p-4 shadow-sm border-2 border-dashed border-[#C94A2A]/30 flex items-center gap-3 active:scale-[0.98] transition-transform"
        >
          <div className="w-12 h-12 rounded-xl bg-[#FDF6EC] flex items-center justify-center">
            <Plus className="w-6 h-6 text-[#C94A2A]" />
          </div>
          ...
          <div className="relative w-full max-w-md bg-white rounded-t-3xl p-6" style={{ animation: 'slideUp 0.3s ease-out' }}>
            ...
            <div className="flex gap-3 mb-5">
              {(['home', 'work', 'other'] as const).map(t => {
                ...
                return (
                  ...
                  >
                    <Icon className={`w-5 h-5 ${iconType === t ? 'text-[#C94A2A]' : 'text-neutral-400'}`} />
                    <span className={`text-xs font-bold ${iconType === t ? 'text-[#C94A2A]' : 'text-neutral-500'}`}>{labels[t]}</span>
                    {iconType === t && <Check className="w-3 h-3 text-[#C94A2A]" />}
                  </button>
                );
              })}
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                ...
                <input
                  {...register('label')}
                  className={`w-full bg-neutral-50 border ${errors.label ? 'border-red-500' : 'border-neutral-200'} rounded-xl px-4 py-3 outline-none focus:border-[#C94A2A] text-sm`}
              ...
              <div>
                ...
                <input
                  {...register('address')}
                  className={`w-full bg-neutral-50 border ${errors.address ? 'border-red-500' : 'border-neutral-200'} rounded-xl px-4 py-3 outline-none focus:border-[#C94A2A] text-sm`}
              ...
              <div>
                ...
                <input
                  {...register('detail')}
                  className="w-full bg-neutral-50 border border-neutral-200 rounded-xl px-4 py-3 outline-none focus:border-[#C94A2A] text-sm"
              ...
              <button
                type="submit"
                className="w-full bg-[#C94A2A] text-white font-bold py-4 rounded-xl mt-2 shadow-lg shadow-[#C94A2A]/30 active:scale-[0.98] transition-transform"
              >
