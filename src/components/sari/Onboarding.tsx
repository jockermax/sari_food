...
import { MapPin, Store, Zap, ArrowRight } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#FDF6EC] flex flex-col">
      <div className="flex justify-between items-center p-5">
        <button
          ...
          className="flex items-center gap-1.5 bg-[#F4A012]/15 px-3 py-1.5 rounded-full"
        >
          <span className="w-2 h-2 rounded-full bg-[#F4A012]" />
          <span className="text-xs font-bold text-[#C94A2A]">Mode démo</span>
        </button>
        <button
          onClick={() => setScreen('role-select')}
          className="text-neutral-500 text-sm font-medium hover:text-[#C94A2A]"
        >
          Passer
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 pb-6">
        <div className="relative w-full max-w-sm aspect-square rounded-3xl overflow-hidden mb-8 shadow-xl">
          ...
          <div 
            className="absolute top-5 left-5 w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg"
          ...
          {slides.map((_, i) => (
            <div
              ...
              className={`h-2 rounded-full transition-all ${
                i === idx ? 'w-8 bg-[#C94A2A]' : 'w-2 bg-neutral-300'
              }`}
            />
          ))}
        </div>
      ...
      <div className="px-6 pb-8">
        <button
          onClick={next}
          className="w-full bg-[#C94A2A] text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 shadow-lg shadow-[#C94A2A]/30 active:scale-[0.98] transition-transform"
        >
